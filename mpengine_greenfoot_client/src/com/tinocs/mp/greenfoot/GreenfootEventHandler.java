 ...
 * Every Actor that needs to be shown on all clients is a MPActor.
 ...
 * MPActor to reflect the changes.
 * 
...
public abstract class GreenfootEventHandler implements ClientEventHandler {
    ...
    public void handleCommand(String command, Client client) {
        if (client.debug()) System.out.println("Attempting to handle command in GreenfootEventHandler " + command);
        ...
        if (cmd.equals(CMD_ADD)) {
            ...
            if (client.debug()) System.out.println("Command not handled by GreenfootEventHandler " + command);
        }
        scan.close();
    }
    ...
    protected void handleSetLocationCmd(String actorId, int x, int y) {
        MPActor ga = getWorld().getMPActor(actorId);
